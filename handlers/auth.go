...
func PostFortniteToken(c * fiber.Ctx) error {
    ...
    if err: = c.BodyParser( & body);
    err != nil {
        return c.Status(fiber.StatusBadRequest).JSON(aid.ErrorBadRequest("Invalid Request Body"))
    }

    if action, ok: = oauthTokenGrantTypes[body.GrantType];
    ok {
        if body.GrantType == "password" && body.Password != "" {
            return action(c, & body)
        } else if body.GrantType == "client_credentials" && body.TokenType != "eg1" {
            return action(c, & body)
        }
    }
...
func GetPublicAccounts(c * fiber.Ctx) error {
    ...
    for _,
    accountIdSlice: = range accountIds {
